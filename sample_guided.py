...
def main():
    ...
    if Path(args.encoder_path).exists() and args.guidance_scale > 0:
        ...
        encoder.eval()
        logger.info(f'Encoder loaded (AUC={enc_ckpt["auc_roc"]})')
    else:
    ...
    if Path(args.plip_path).exists() and args.guidance_scale > 0:
        ...
        # Type-aware coords — разбиваем по типам взаимодействий
        if interactions:
            typed_pharma_coords = {}
            ...
            logger.info(f'Loaded type-aware pharmacophore coords ({len(interactions)} total)')
    else:
