...
class ScorePosNet3D(nn.Module):
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
        ...
        # ── Guidance параметры из атрибутов модели ──────────────────────
        guidance_scale       = getattr(self, 'guidance_scale', 0.0)
        guidance_start_step  = getattr(self, 'guidance_start_step', 200)
        beta           = getattr(self, 'beta',           0.5)
        gamma          = getattr(self, 'gamma',          0.3)
        encoder        = getattr(self, 'encoder',        None)
        z_refs         = getattr(self, 'z_refs',         None)
        pharma_coords  = getattr(self, 'pharma_coords',  None)
        # ─────────────────────────────────────────────────────────────────
